...
export default function LottoRow({ lotto, highlighted = false }: ArticoloProps) {
    
    const searchParams = useSearchParams()
    const params = new URLSearchParams(searchParams.toString())
    params.set("p", String(lotto.id))
    const editHref = `/lotti?${params}`
    ...
    return (
        <div className={`h-15 flex transition-color duration-100 ${highlighted ? "bg-yellow-300/50" : "hover:bg-surface-raised bg-surface"} items-center gap-x-4 md:gap-x-0 justify-between shrink-0 rounded-2xl px-5 shadow-md border-border truncate`}>
            <h1 className="w-full font-extrabold text-md lg:text-xl text-ellipsis truncate">{lotto.articolo.nome}</h1>
            <h1 className="text-sm w-full text-text/75 hidden lg:block">{String(lotto.quantitaOrdinata)} {lotto.unita.tipo} ({lotto.quantitaOrdinata / lotto.articolo.quantitaRecipiente}) </h1>
            <h1 className="text-sm w-full text-text/75 hidden lg:block">{String(lotto.dataLotto).split("T")[0]}</h1>
            <h1 className="text-text w-full text-md text-ellipsis truncate hidden lg:block">{String(lotto.dataScadenza).split("T")[0]}</h1>
