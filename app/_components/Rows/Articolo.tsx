...
export default function ArticoloRow({ articolo, setEdit }: ArticoloProps) {
    return (
        <div key={articolo.id} className="h-15 flex hover:bg-surface-raised transition-color duration-100  bg-surface items-center gap-x-4 md:gap-x-0 justify-between shrink-0  rounded-2xl px-5 shadow-md border-border truncate ">
            ...
            <div className="lg:w-50 shrink-0 flex items-center gap-x-5">
                <div>
                    <button onClick={() => setEdit(articolo.id)} className="hidden  text-text items-center lg:flex h-[75%]  rounded-lg font-bold hover:cursor-pointer hover:scale-105 transition-transform duration-150"> Edit</button>
                    <Link href={`/articoli/${articolo.id}`} className="flex lg:hidden text-text items-center h-[75%]  rounded-lg font-bold hover:cursor-pointer hover:scale-105 transition-transform duration-150">Edit</Link>
                </div>
